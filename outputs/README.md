...
| `leak_ab_all12_v1` | **verified 12-section label-leak A/B (2026-08-19)** — historical record; measurement script removed | `outputs/mp_mnca/leak_ab_all12_v1/{leak_ab_all_sections.csv,summary.json,prepared/}` | Both arms per section, identical seed/config/epochs, only the pseudo-label source differing. Leak mean **+0.2615** ARI, median +0.3327, range [−0.2294, +0.4283], positive on 11/12. Negative on 151675 (−0.2294): removing same-layer negatives can starve the contrastive objective, so the leak is not purely additive. Absolute values are thread-config sensitive (`OMP_NUM_THREADS=4`); the within-run delta is the sound quantity. The leak is fixed in `src/mp_mnca/` as of the `SM/geopatch` merge, which removes the negative mask entirely |
...
| `20260819_mp_mnca_phase1_orig_20ep_all12` | verified MP-MNCA Phase 1, original code restored | `outputs/ablations_phase1_orig_20ep_all12/phase1_20ep/` | Original (un-refactored) Phase 1, 20 epochs, all 12 sections; best/max refined ARI per section: mean 0.5195, median 0.5185. 151507 reproduced 0.5788 exactly, confirming the earlier 0.538 drop was caused by the cross-attention refactor; `phase1.py` byte-identical to commit `c8235a5` |
| `graphst_seed41_all12_200ep` | verified matched-epoch GraphST | `outputs/prior_models/graphst_seed41_all12_200ep/` | GraphST at 200 epochs (native is 600): mean refined ARI 0.4765, median 0.4895. Drops from 600-epoch 0.5324/0.5280, confirming GraphST needs 600 epochs. At the matched 200-epoch budget, 20-epoch MP-MNCA Phase 1 (0.5195/0.5185) beats GraphST, STAIG (0.473/0.485), and SpaGCN (0.497/0.486) and ties 1000-epoch MuCoST (0.522/0.519) |
| `mucost_seed2023_all12_200ep` | verified matched-epoch MuCoST | `outputs/prior_models/mucost_seed2023_all12_200ep/` | MuCoST at 200 epochs (native is 1000): mean refined ARI 0.4555, median 0.4461. Drops from 1000-epoch 0.5221/0.5186, confirming MuCoST needs 1000 epochs. Final matched-200ep table: MP-MNCA Phase 1 (20ep) 0.5195/0.5185 > SpaGCN 0.4969/0.4856 > GraphST 0.4765/0.4895 > STAIG 0.4730/0.4850 > MuCoST 0.4555/0.4461 |
